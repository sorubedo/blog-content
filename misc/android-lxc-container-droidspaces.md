...
[[Droidspaces]] 是一个基于 Linux 内核 namespace 的轻量容器运行时。单个静态二进制，不到 300KB，zero dependency。它直接在 Android 上创建真正的容器 —— 有独立的 PID 树、独立的挂载表、独立的 cgroup 层级，systemd 作为 PID 1 完整启动。
...
而我，现在就在手机的 Debian 容器里跑着 [[Claude Code]] + [[cc-connect]]，挂载了手机存储，用聊天软件发指令写文档，Obsidian 实时刷新预览 —— 一条龙。
...
[[Droidspaces]] 的使用体验出奇地好：

### 安装就是下载一个 APK

GitHub Releases 直接下，装上就能用。Android 端有完整的 GUI，容器创建、配置、启动、进终端，全部点几下就行。

### 真正的 FHS Linux 发行版

我拉了个 Debian rootfs，启动后：
...
- 容器里的 [[Claude Code]] 可以直接读写手机上的文件
- 用 [[Obsidian]] 打开同一个目录，实时预览和编辑
...
但这些问题，Droidspaces 的文档基本都覆盖了。它的 built-in requirements checker 能直接告诉你缺什么内核特性，不用自己对着 dmesg 猜。
