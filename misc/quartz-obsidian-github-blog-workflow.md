...
> [!warning] 前置步骤：创建 GitHub 仓库
> 本文假设你已经在 GitHub 上创建好了 `blog` 和 `blog-content` 两个仓库（一个放框架，一个放内容）。仓库创建步骤请参考 Quartz 官方教程：[Setting up your GitHub repository](https://quartz.jzhao.xyz/setting-up-your-GitHub-repository)，写得够清楚了，我就不抄一遍了。git 和 github 总得会用吧（
...
> [!note]
> Obsidian 的 `.obsidian/` 配置目录也在 `blog-content` 里，但在 `quartz.config.ts` 中默认有配置 `ignorePatterns` ，不用担心被构建。
